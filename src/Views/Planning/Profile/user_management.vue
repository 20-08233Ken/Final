...
<template>
  <div class="w-10/12 flex flex-col px-3vw py-3vw bg-white shadow-card2">
    ...
    <v-data-table :headers="headers" :items="allUserData" class="elevation-1 mt-4" items-per-page="10"
      loading-text="Loading... Please wait" style="width: 100%; overflow-x: scroll">
      <template v-slot:item.action="{ item }">
        <span class="flex items-center">
          <!-- Edit User -->
          <v-dialog max-width="900">
            <template v-slot:activator="{ props: activatorProps }">
              <v-btn v-bind="activatorProps" text="Open Dialog" variant="flat" size="small" @click="openUser(item)">
            ...
            <template v-slot:default="{ isActive }">
              <v-card title="">
                <v-card-text>
                  <h3 class="w-full font-bold font-Subheader bg-gray-700 text-white py-4 px-4 mt-1">
                    Modify User Profile
                  </h3>
                  ...
                  <Form class="mt-4" @submit="submitUpdate(updatedData.id)">
                    ...
                    <Field type="text" name="company_id" placeholder="Type here"
                      class="input mt-2 input-bordered w-full" style="border: 1px solid #d2d2d2" :rules="validateData"
                    ...
                    <Field type="text" name="fname" placeholder="Type here" class="input mt-2 input-bordered w-full"
                      style="border: 1px solid #d2d2d2" :rules="validateData" v-model="updatedData.firstname" />
                    ...
                    <Field type="text" name="lname" placeholder="Type here" class="input mt-2 input-bordered w-full"
                      style="border: 1px solid #d2d2d2" :rules="validateData" v-model="updatedData.lastname" />
                    ...
                    <Field type="text" name="mname" placeholder="Type here" class="input mt-2 input-bordered w-full"
                      style="border: 1px solid #d2d2d2" :rules="validateData" v-model="updatedData.middlename" />
                    <ErrorMessage name="mname" class="error_message" />

                    <p class="text-0.9 font-Subheader text-gray-500 mt-4">
                      Username
                    </p>
                    <Field type="text" name="username" placeholder="Type here" class="input mt-2 input-bordered w-full"
                      style="border: 1px solid #d2d2d2" :rules="validateData" v-model="updatedData.username" />
                    ...
                    <Field type="password" name="confirm_password" placeholder="Type here"
                      class="input mt-2 input-bordered w-full" style="border: 1px solid #d2d2d2"
                      v-model="updatedData.confPassword" />
                    <!-- <ErrorMessage name="confirm_password" class="error_message" /> -->

                    <p class="flex item-center gap-2 text-Red-Darken font-Subheader mt-8">
                      Designation
                    </p>
                    ...
                    <Field as="select" name="campus" class="w-full select select-bordered mt-2"
                      style="border: 1px solid #d2d2d2" :rules="validateData" v-model="updatedData.campus_id"
                      @change="changeCollege(updatedData.campus_id)">
                    ...
                    <Field as="select" name="office" class="w-full select select-bordered mt-2"
                      v-model="updatedData.office_id" style="border: 1px solid #d2d2d2">
                    ...
                    <Field as="select" name="postion" class="select select-bordered mt-2 w-full"
                      style="border: 1px solid #d2d2d2" :rules="validateData" v-model="updatedData.position_id">
          ...
          <v-dialog max-width="500">
            <template v-slot:activator="{ props: activatorProps }">
              <v-btn v-bind="activatorProps" text="Open Dialog" variant="flat" size="small">
            ...
            <template v-slot:default="{ isActive }">
              <v-card title="">
                <v-card-text>
                  <h3 class="w-full bg-Red-Darken text-white px-4 py-2 text-lg font-Subheader">
                    Warning!!
                  </h3>
                ...
                <v-card-actions>
                  ...
                  <form class="w-full px-6 pb-4 flex justify-end gap-4">
                    ...
                    <v-btn text="Delete " @click="deleteUser(item.userID, (isActive.value = false))"
                      class="bg-red-darken-3"></v-btn>
                  </form>
                </v-card-actions>
              </v-card>
            </template>
